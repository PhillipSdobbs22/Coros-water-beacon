...
export_data.c                dumps firmware behaviour as JSON for the demo
make_gif.py                  renders the animation above from that JSON
index.html                   the interactive demo, self-contained
state-machine.svg            the state machine
...
<p align="center"><img src="state-machine.svg" alt="Beacon state machine" width="620"></p>
...
make data    # regenerate the demo's data from the firmware
```

C11, no dependencies beyond libm for the test bench. It builds warning-clean under `-Wall -Wextra -Wshadow
-Wconversion -Wsign-conversion -Werror` on Linux and macOS. Rebuilding the animation additionally needs
